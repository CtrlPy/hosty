...
def check_root():
    if os.geteuid() != 0:
        print("This utility must be run as root. Please run with sudo or as root.")
        sys.exit(1)
...
def main():
    ...
    # Remove the BoxAdapter to allow dynamic height adjustment
    # left_pile_box = urwid.BoxAdapter(urwid.Filler(left_pile, valign="top"), height=20)
    left_pile_box = urwid.Filler(left_pile, valign="top")
    ...
    host_list_box = urwid.LineBox(padded_host_list, title="List of host names")
...
def handle_input(key):
    ...
    if key == "tab":
        focus_position = columns.focus_position
        if focus_position == 0:
